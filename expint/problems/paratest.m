function problem = paratest(varargin)
% PARATEST - 
%
%             y_t = y_xx - y y_x + Phi
%
% SYNOPSIS:
%   problem = paratest;
%   problem = paratest(problemdata);
%   problem = paratest('f1', v1, ..., 'fn', vn);
%
% PARAMETERS:
...
%
% RETURNS:
%   problem  - Structure containing problem specific parameters of the PARATEST
...
%
% $Revision: 1.3 $  $Date: 2005/10/13 13:37:40 $
...
switch lower(IC);
   ...
   case {'exactic'}
      ...
      problem.exactsolution = 'paratest_exact';
   otherwise
      if strmatch('reg', lower(IC)),
         ...
         error('paratest:invalidic', 'Unknown IC supplied');
      end
end
...
problem.N = 'paratest_N';

% Postprocessing of the data
problem.postprocessing = 'trivial_post';

% The function Lu + N(u,t), needed for ode15s.
problem.LplusN = 'paratest_LplusN';

% Descriptive name to be used in plots or filenames.
problem.problemname = ['Parabolic Test, ND=', int2str(problem.ND), ...
                       ', IC: ', problem.ICname];

problem.problemnametex = ['Parabolic Test, $\mathit{ND}=', int2str(problem.ND), ...
